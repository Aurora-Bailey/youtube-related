...
async function start () {
  ...
  console.log(['Date', 'Channel ID', 'Channel Name', 'Subscribers', 'Sum', 'Average', 'Count'].join(','))
  for (var i = 0; i < numChunks; i++) {
    try {
      ...
      console.log([date.toJSON(), largest[0].info.id, largest[0].info.snippet.title, largest[0].info.statistics.subscriberCount, sum, Math.round(sum / subscriberCount.length), subscriberCount.length].join(','))
      subscriberCount = ''
    } catch (e) {
    }
  }
